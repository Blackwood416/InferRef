...
The isolated testcase in `testcase/` represents a standard rotary-position
embedding boundary. The NumPy engine in `engine.py` completes successfully but
does not numerically match the reference.
...
- Do not edit the testcase, adapter, task, or evaluation metadata.
...
The intended operation is ordinary half-split RoPE, not interleaved-pair RoPE.
